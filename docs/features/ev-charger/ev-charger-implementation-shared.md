...
- **`slot_stop` transition behavior**: when a slot with EV charging ends and the next slot has no EV action for that same appliance, Helman turns off only the charge switch and does not write `use_mode` or `eco_gear`. If the charger later reflects a different mode as a side effect of stopping, that charger-driven drift is acceptable. When no EV action exists in a slot, the system does nothing for that appliance.
...
Story 06 consolidates this into a shared coordinator computation so the pipeline runs once per cache cycle. Until then, appliance projections must still be recalculated only from unmodified upstream inputs and must never consume already-adjusted downstream outputs.

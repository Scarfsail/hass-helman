...
| `forecast_history.py` | Build the `TrainerSample` list from the live archive (`solar_forecast_history.py`), one per past day, and the inspector's forecast curve for any day. The v1 Option A capture rule -- first Recorder state after local midnight -- was retired by #178. | `load_trainer_samples(store, cfg, now)`, `async load_forecast_points_for_day(...)` |

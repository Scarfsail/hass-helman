...
Import-price bands are classified per local calendar day, but a continuous price window may span midnight and is treated as one window by `charge_from_grid`. Before an expensive window it first preserves battery energy by stopping discharge at the latest safe time, then charges only the simulated residual in the latest feasible cheap slots. The charge target is chosen so the battery still enters the expensive window at the needed level after draining through any hours between the cheap band and the window, up to the battery's max SoC. Two extra 15-minute charge opportunities are included by default as a safety margin; they do not raise the target. When even a full battery cannot maintain the reserve, the result remains best-effort and reports the existing cap/capacity limit.

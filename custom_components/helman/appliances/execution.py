...
from ..scheduling.runtime_status import ApplianceRuntimeStatus
...
class SwitchEntityController:
    def __init__(self, entity_id: str) -> None:
        ...
        if not separator or not object_id or domain != "switch":
            raise ScheduleExecutionUnavailableError(
                "EV charge entity must use the switch domain"
            )
    ...
    def read_state(self, hass: HomeAssistant) -> Any:
        ...
        if state is None:
            raise ScheduleExecutionUnavailableError(
                f"EV charge entity '{self.entity_id}' is not available"
            )
        ...
        if (
            ...
            raise ScheduleExecutionUnavailableError(
                f"EV charge entity '{self.entity_id}' is unavailable"
            )
        return state
    ...
    async def async_turn_on(self, hass: HomeAssistant) -> None:
        try:
            ...
            raise ScheduleExecutionUnavailableError(
                f"Failed to turn on EV charge entity '{self.entity_id}'"
            ) from err
    ...
    async def async_turn_off(self, hass: HomeAssistant) -> None:
        try:
            ...
            raise ScheduleExecutionUnavailableError(
                f"Failed to turn off EV charge entity '{self.entity_id}'"
            ) from err
...
class EvChargerExecutor:
    ...
    async def async_execute(
        ...
        *,
        ...
        if action is None:
            if memory is None or not memory.last_charge_enabled:
                return None, None
            if (
        ...
        if (
            ...
            return (
                ...
                ApplianceExecutionMemory(
                    ...
                    last_charge_enabled=bool(action["charge"]),
    ...
    async def _async_apply_action(
        ...
        *,
        appliance: EvChargerApplianceRuntime,
        action: ApplianceScheduleActionDict,
        reference_time: datetime,
        ...
        try:
            switch_controller = SwitchEntityController(appliance.charge_entity_id)
...
class AppliancesExecutor:
    ...
    async def async_execute(
        ...
        *,
        ...
        for appliance in registry.appliances:
            runtime, memory = await self._ev_executor.async_execute(
                appliance=appliance,
    ...
    async def async_restore_active_slot(
        ...
        *,
        ...
        for appliance in registry.appliances:
            runtime = await self._ev_executor.async_disable_active_action(
                appliance=appliance,
                action=active_actions.get(appliance.id),
                reference_time=reference_time,
            )
